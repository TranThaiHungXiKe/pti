...
class SorTing (QMainWindow,Ui_sorting):
    def __init__ (self):
        ...
        #self.pushButton_thitkho.clicked.connect(self.book_thit_kho)
         ...
         self.btns_sort_name.clicked.connect(self.setTypeName)
         self.btns_sorts_core.clicked.connect(self.setTypeScore)

    def setTypeName(self):
        self.typeSort = "name"
        self.loadDataObjects()
    def setTypeScore(self):
        self.typeSort = "score"
        self.loadDataObjects()
       

    def searchFood(self):
        self.valueSearch = self.ValueSearch.text()
        dataSearch = self.l.searchfoodbyname(self.valueSearch) #--> list object
        self.loadDataObjects()
        
    def searchFoodByVoice(self):
         self.valueSearch = speech_to_text()
    ...
    def loadDataObjects(self):
          self.setupUi(self)
          self.l = food()
          if self.typeSort == "null":
              self.l = self.l
          elif self.typeSort == "name":
              self.l.sortFoodByName()
          elif  self.typeSort == "score":
              self.l.sortFoodByScore()
          ...
          for x in self.l.getAllfood():
            if self.valueSearch in x.getName():
